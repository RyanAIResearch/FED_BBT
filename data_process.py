...
class data_processor():
    def __init__(self, args) -> None:
        ...
        global cache_fn
        cache_fn = f"caches/data_{self.model_name.replace('/', '-')}_{self.task_name}_{self.n_prompt_tokens}_{self.seed}.pt"
    ...
    def get_data(self):
        ...
        if self.cat_or_add == 'cat':
            ...
            data_bundle = self.DataLoader[self.task_name](tokenizer=self.tokenizer, n_prompt_tokens=self.n_prompt_tokens).my_load(splits)
        return data_bundle
...
def split_data(args, train_data, dev_data):
    ...
    if args.iid == 1:
        for i in range(args.num_users):
            user_dict_train[i] = set(np.random.choice(train_data_idxs, num_items_train,
                                                replace=False))
            user_dict_dev[i] = set(np.random.choice(dev_data_idxs, num_items_dev,
                                                replace=False))
...
def perturb_dataset(args, dataset, config):
    ...
    if isinstance(dataset, dict):
        ...
        replace_mask = torch.bernoulli(args.perturb_rate*torch.tensor(dataset['attention_mask'].get(range(len(dataset))))).long()
        preserve_mask -= replace_mask
        pert_dataset['input_ids'].content = (input_content * preserve_mask + random_text * replace_mask).tolist()
        pert_dataset['attention_mask'].content = torch.tensor(pert_dataset['attention_mask'].get(range(len(dataset)))).tolist()
