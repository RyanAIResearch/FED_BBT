...
with open("/workspace/Black-Box-Tuning/results/agnews/prompt/fedavg_prompt_iid0_alpha1.0_popsize{}.pkl".format(lp), 'rb') as f:
    prompt_noniid_dict = pickle.load(f)

with open("/workspace/Black-Box-Tuning/results/agnews/prompt/fedavg_prompt_iid1_alpha0.5_popsize{}.pkl".format(lp), 'rb') as f:
    prompt_iid_dict = pickle.load(f)


for c in range(10):
    prompt_noniid_dict[c] = np.vstack(prompt_noniid_dict[c])
    prompt_iid_dict[c] = np.vstack(prompt_iid_dict[c])
...
y = []
prompt_noniid = np.concatenate([prompt_noniid_dict[1], prompt_noniid_dict[5], prompt_noniid_dict[7]], axis=0)
...
prompt_iid = np.concatenate([prompt_iid_dict[1], prompt_iid_dict[5], prompt_iid_dict[7]], axis=0)
...
fig = plot.get_figure()
fig.savefig(f'fedavg_tsne_{lp}.png')