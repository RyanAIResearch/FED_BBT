...
for e in range(args.epochs):
    ...
    for idx in idxs_users:
        ...
        if args.p_tuning == 1:
            optimizer = torch.optim.Adam(params=local_prefix_encoder.parameters(), lr=1e-5)
        else:
        ...
        local_train_data = train_data[np.array(list(dev_sample_idxs)).tolist()]
        ...
        for le in range(args.local_epochs):
            ...
            # es.disp()
            if le % 10 == 0:
                print('Local evaluate on test data...')
                local_prompt = local_prefix_encoder(prefix_tokens)
                test_acc = model_forward_api.eval(prompt_embedding=local_prompt, test_data=test_data)
                print('Local test acc @ epoch {}: {}'.format(le, round(test_acc, 4)))
        end_time = time.time()
