...
if args.init_prompt_path is None:
    global_prompt_embedding = None
else:
    global_prompt_embedding = torch.load("/workspace/Black-Box-Tuning/results/llama/sst2/larger_global_pop_new_sigma_pert/fl_iid{}_prompt.pt".format(args.iid))[0]
...
for e in range(args.epochs):
    ...
    for idx in idxs_users:
        ...
        for le in range(args.local_epochs):
            for batch_x, batch_y in dataloader_train:
                ...
                loss, _ = model_forward_api.eval(global_prompt_embedding, train_data=batch_x, train_label=batch_y)
                loss.backward()
                if args.model_name in ['llama2']:
    ...
    # for name,para in model_forward_api.model.named_parameters():
    #     if 'score' in name:
    #         torch.save(para.cpu().detach(), 'results/llama/sst2/fedprompt/fl_iid{}_score_state_dict.pt'.format(args.iid))
