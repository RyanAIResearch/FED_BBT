...
for e in range(args.epochs):
    ...
    for idx in idxs_users:
        ...
        global_Cs.append(local_es.sm.C.copy())
        global_means.append(local_es.mean)
        global_sigmas.append(local_es.sigma)
        # fitlog.finish()
        client_api_setting_list[idx] = model_forward_api.client_record()

    global_es.mean = np.average(global_means,axis=0)
    global_es.sigma = np.average(global_sigmas,axis=0)
    global_es.sm.C = np.average(global_Cs,axis=0)
    global_es.sm.update_now(-1)
    global_es._updateBDfromSM()
    ...
    print('Global test acc: {}'.format(round(test_acc, 4)))
    print('Check sigma: {}'.format(global_es.sigma))
    global_api_setting = model_forward_api.client_record()
