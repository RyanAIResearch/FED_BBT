    ...
    def eval(self, prompt_embedding=None, test_data=None):
        ...
        if isinstance(test_data, DataSet):
            ...
            test_tester = Tester(data=test_data, model=self.model, metrics=self.metric, batch_size=batch_size,
                                 num_workers=1, device=device, use_tqdm=False)
...
while not es.stop():
    ...
    # es.disp()
    if (es.countiter+1) % args.eval_every == 0 :
        print('Evaluate on test data...')
        model_forward_api.best_prompt = es.mean
        test_acc = model_forward_api.eval(test_data=test_data)
        print('Test acc @ {} test: {}'.format(int(es.countiter),round(test_acc, 4)))
        print('Check sigma @ {} sigma: {}'.format(int(es.countiter), es.sigma))
end_time = time.time()
