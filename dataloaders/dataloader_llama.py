...
class SST2Loader(Loader):
    ...
    #         offset = 1000
    #         prompt = self.tokenizer.decode(list(range(offset, offset + self.n_prompt_tokens)))
    #         example['input_text'] = '%s . %s . The positive or negative sentiment of this sentence is' % (prompt, example['sentence'])
    #         example['target_text'] = self.label2text[example['label']]
    #     else:
    #         example['input_text'] = '%s . The positive or negative sentiment of this sentence is' % example['sentence']
    #         example['target_text'] = self.label2text[example['label']]
    #     return example
    ...
    def convert_examples(self, example):
        if self.n_prompt_tokens > 0:  # use randomly selected words as initial prompt
            offset = 1000
            prompt = self.tokenizer.decode(list(range(offset, offset + self.n_prompt_tokens)))
            example['input_text'] = 'What is the sentiment of this sentence: %s . %s .' % (prompt, example['sentence'])
            example['target_text'] = self.label2text[example['label']]
        else:
            example['input_text'] = 'What is the sentiment of this sentence: %s .' % example['sentence']
            example['target_text'] = self.label2text[example['label']]
        return example
