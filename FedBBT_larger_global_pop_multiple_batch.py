...
for e in range(args.epochs):
    ...
    for idx in idxs_users:
        ...
        while not local_es.stop():
            ...
            if parallel:
                fitnesses_orig = model_forward_api.eval_multi_batch(solutions)
                fitnesses_pert = model_forward_api.eval_perturb_multi_batch(solutions)
                if args.perturb != 0:
                ...
                if args.perturb != 0:
                    fitnesses = [model_forward_api.eval_multi_batch(x)/model_forward_api.eval_perturb_multi_batch(x) for x in solutions]
                else:
                    fitnesses = [model_forward_api.eval_multi_batch(x) for x in solutions]
        ...
        if parallel:
            ...
            fitnesses_orig = np.array([model_forward_api.eval_multi_batch(x) for x in solutions])
            fitnesses_pert = np.array([model_forward_api.eval_perturb_multi_batch(x) for x in solutions])
            if args.perturb != 0:
                fitnesses = fitnesses_orig/fitnesses_pert
            else:
                fitnesses = fitnesses_orig
